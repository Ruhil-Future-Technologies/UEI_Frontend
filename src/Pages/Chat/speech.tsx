...
const useTextToSpeech = () => {
    ...
    const textToSpeech = async (texts: string, index: number): Promise<boolean> => {
        stopSpeech(index); // Stop any previous playback
        const text = Array.isArray(texts) ? texts[0] : texts;
        const chunks = splitTextIntoChunks(text, MAX_CHARS);
        let currentAudioIndex = 0;

        const playChunk = async (): Promise<void> => {
            if (currentAudioIndex >= chunks.length) {
                delete audioInstances[index];
                return;
            }
        ...
        // Return a promise that resolves after speech has finished
        return new Promise((resolve) => {
            const interval = setInterval(() => {
                if (currentAudioIndex >= chunks.length) {
                    clearInterval(interval);
                    resolve(true); // Resolve once speech is complete
                }
