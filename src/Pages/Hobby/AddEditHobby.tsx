...
const AddEditHobby = () => {
  ...
  const handleChange = async (
    ...
    if (
      formRef?.current?.errors?.[fieldName as keyof IHobbyForm] !== undefined
    ) {
      formRef?.current?.setFieldError(
        fieldName,
        formRef?.current?.errors?.[fieldName as keyof IHobbyForm],
      );
    ...
    if (id) {
      hobbySchema = Yup.object().shape({
        hobby_name: Yup.string()
          ...
          .matches(
            /^[a-zA-Z\s]*$/,
            'Please enter a valid hobby name only characters allowed.',
          )
          .test('unique', 'Hobby name already exists', function (value) {
            ...
            const exists = dataHobby.some(
              (inst) =>
                inst.hobby_name &&
                inst.hobby_name.toLowerCase() === value.toLowerCase(),
            );
