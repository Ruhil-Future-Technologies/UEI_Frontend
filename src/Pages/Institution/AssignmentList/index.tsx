...
import { ASSIGNMENT_LIST_COLUMNS, IClass } from "../../../Components/Table/columns";
import { QUERY_KEYS_TEACHER, QUERY_KEYS_COURSE, QUERY_KEYS_CLASS, QUERY_KEYS_SEMESTER, QUERY_KEYS_SUBJECT_SCHOOL, QUERY_KEYS_SUBJECT, QUERY_KEYS_ASSIGNMENT } from "../../../utils/const";
import FullScreenLoader from "../../Loader/FullScreenLoader";

const AssignemntList = () => {
    ...
    const ASSIGNMENT_LIST = QUERY_KEYS_ASSIGNMENT.GET_ASSIGNMENTS_LIST;
    const columns = ASSIGNMENT_LIST_COLUMNS
    ...
    const [assignmentListFilter, setAssignmentListFilter] = useState<any[]>([]);
    ...
    const listOfAssignments = () => {
        setLoading(true)
        getData(ASSIGNMENT_LIST).then((assignment) => {
            setLoading(false)
            if (assignment?.status) {
                setAssignmentList(assignment?.data);
                setAssignmentListFilter(assignment?.data);
            }
        })
    }
    const listOfTeacher = () => {
   ...
   const handleChanges = (event: any) => {
        ...
        if (name == "teacher_id") {
            ...
            const filteredQuizList = assignmentList.filter((assignment) => assignment?.created_by == techerValue?.user_uuid)
            setAssignmentListFilter(filteredQuizList);
        }
        if (name == "course_id") {
            ...
            const filteredQuizList = assignmentList.filter((assignment) => {
                if (!assignment?.course_semester_subjects) {
                    return false;
                }
                const teacherCourses=Object.keys(assignment?.course_semester_subjects);
                return assignment?.created_by == selecteTeacher?.user_uuid && teacherCourses.includes(String(value))
            })
            setAssignmentListFilter(filteredQuizList);
        }
        if (name == "semester_id") {
            ...
            const filteredQuizList = assignmentList.filter((assignment) =>{
              if(!assignment?.course_semester_subjects){
               ...
               const semesters=Object.values(assignment?.course_semester_subjects);
              ...
              return (courseListForteacher.includes(String(selectedCourse))&& totalSemester.includes(String(value)) && assignment?.created_by==selecteTeacher?.user_uuid)
            })
             setAssignmentListFilter(filteredQuizList);
        }
        ...
        if (name == "class_id") {
            ...
            const filteredQuizList = assignmentList.filter((assignment) => {
                if (!assignment?.class_stream_subjects) {
                    return false; // Explicitly return false
                }
                const classesIds = Object.keys(assignment?.class_stream_subjects);
                return (classesIds.includes(String(value)) && assignment?.created_by == selecteTeacher?.user_uuid)
            })
            setAssignmentListFilter(filteredQuizList);
        }
        if (name == "subject_id") {
            ...
            if (entityType == "college") {
                const filteredQuizList = assignmentList.filter((assignment) => {
                    const semestervale = Object.values(assignment?.course_semester_subjects);
                    if (!assignment?.course_semester_subjects) {
                        return false; // Explicitly return false
                    }
                    ...
                    const courseIds = Object.keys(assignment?.course_semester_subjects);
                    return (courseIds.includes(String(selectedCourse)) && subjectListT.includes(String(selectedSubjectName)) && semesterList.includes(String(selectSemester)) && assignment?.created_by == selecteTeacher?.user_uuid)
                })
                setAssignmentListFilter(filteredQuizList);
            } else {
                
                if (selectClass?.class_name == "class_11" || selectClass?.class_name == "class_12") {
                    const filteredQuizList = assignmentList.filter((quiz) => {
                    ...
                    const filteredQuizList = assignmentList.filter((assignment) => {
                        const streamsvale = Object.values(assignment?.class_stream_subjects);
                        if (!assignment?.class_stream_subjects) {
                            return false; // Explicitly return false
                        }
                        let subjectListT: string[] = [];
                        ...
                        //console.log(streamList);
                        const classesIds = Object.keys(assignment?.class_stream_subjects);
                        return (classesIds.includes(String(selectClass?.id)) && subjectListT.includes(String(selectedSubjectName)) && assignment?.created_by == selecteTeacher?.user_uuid)
                    })
                    setAssignmentListFilter(filteredQuizList);
                }
            }
        ...
        if (name == "stream") {
            ...
            const filteredQuizList = assignmentList.filter((assignment) => {
                ...
                const streamsvaleues = Object.values(assignment?.class_stream_subjects);
                let streamList: string[] = [];
                ...
                const classesIds = Object.keys(assignment?.class_stream_subjects);
                return (classesIds.includes(String(selectClass?.id)) && streamList.includes(String(value)) && assignment?.created_by == selecteTeacher?.user_uuid)
            })
            setAssignmentListFilter(filteredQuizList);
        }
        ...
        <div className="main-wrapper">
            <div className="main-content">
                ...
                <Typography variant="h6" sx={{ m: 2 }}>
                    <div className="main_title"> Assignment List</div>
                </Typography>
                <Box
                    sx={{
                        display: "flex",
