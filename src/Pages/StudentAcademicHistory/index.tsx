    ...
    getData("/institution/list")
      .then(async (response: any) => {
        if (response.status === 200) {
            const filteredData = await response?.data?.filter((item:any) => item?.is_active === 1);
...
const saveAcademicHistory = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  const isDateValid = (starting_date: string, ending_date: string) => {
    return dayjs(starting_date).isBefore(dayjs(ending_date)) || dayjs(starting_date).isSame(dayjs(ending_date));
  };
...
const handleInputChange1 = (index: number, field: keyof Boxset, value: any) => {
          ...
          <><FormControl sx={{ m: 1, minWidth: 180, width: "100%" }}>
                  {boxes1.map((box, index) => (
                    <TextField
                      ...
                      onChange={(e) =>
                        handleInputChange1(index, 'Institute_Name_Add', e.target.value)
                      }
