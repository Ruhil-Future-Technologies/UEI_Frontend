...
const AssignmentModal: React.FC<Props> = ({
  ...
  const [selectedAssignmentKey, setSelectedAssignmentKey] =
    useState<string>('');
  const [expanded, setExpanded] = useState<number | false>(false);
  const [assignmentSets, setAssignmentSets] =
    useState<AssignmentCollection | null>(null);
  ...
  const handleRegenerate = () => {
    ...
    const assignment_payload = {
      ...payload,
      answer: assignmentSets?.questions,
    };

    postDataJson(ASSIGNMENT.GENERATE_AI_ASSIGNMENT, assignment_payload).then(
      (response) => {
        setAssignmentSets(response);
        setLoading(false);
      },
    );
  };
    ...
    <>
      {loading ? (
        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
          ...
          >
            <div>Preview Assignments</div>

            {!isEdit && (
              <IconButton className="btn btn-secondary text-white">
                <RefreshIcon onClick={handleRegenerate} />
              </IconButton>
            )}
          </DialogTitle>
          ...
          <DialogContent>
            {assignmentSets &&
              Object.values(assignmentSets)?.map((assignment, index) => (
                <Accordion
                  key={index}
                ...
                >
                  <AccordionSummary
                    ...
                    } // Click header selects radio
                  >
                    <Box display="flex" alignItems="center" width="100%">
                      <Radio
                        checked={
                          selectedAssignmentKey ===
                          Object.keys(assignmentSets)[index]
                        }
                        onClick={(e) => {
