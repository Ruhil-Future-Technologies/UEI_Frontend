...
import { Button, Checkbox, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, FormControlLabel, ListItemText, MenuItem, Radio, RadioGroup, Select, SelectChangeEvent, TextField } from "@mui/material";
...
import { QUERY_KEYS, QUERY_KEYS_CLASS, QUERY_KEYS_COURSE, QUERY_KEYS_SUBJECT, QUERY_KEYS_SUBJECT_SCHOOL, /* QUERY_KEYS_ROLE*/ } from "../../utils/const";
...
const TeacherRegistrationPage = () => {
    ...
    const handleSelect = (event: SelectChangeEvent) => {
        ...
        if (name === "institution_id") {
            const filteredDta = dataCourse.filter((item) => String(item.institution_id) === value)
            FilteredDataCourse(filteredDta);
        }
        if (name === "entity_id") {
            const filteredInstitute = dataInstitute.filter((item) => String(item.entity_id) === value)
            setFiteredInstitute(filteredInstitute);
        }
        if(name==="school_name"){
          const selectedSchool=dataInstitute.find((item)=>String(item.id)===value)?.institution_name;
    ...
    const validation = (name: string, value: string) => {
        setError({
            ...
            email_id_error: name === "email_id" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value),
        ...
        teacher.dob === dayjs('dd-mm-yyyy') ? setdobset_col(true) : setdobset_col(false)
    }
    ...
    const handleSubmit = () => {
        setError({
            ...
            designation_role_error: false,
            entity_error: false,
            class_id_error: selectedEntity === "School" && teacher.class_id === "" ? true : false,
            course_id_error: selectedEntity === "College" && teacher.institution_id === "" ? true : false,
            institution_id_error: selectedEntity === "College" && teacher.institution_id === "" ? true : false,
            school_name_error: selectedEntity === "School" && teacher.school_name === "",
        })
        ...
        const isSchoolValid =
            selectedEntity === "School" ?
                (!error.class_id_error && !(teacher.class_id === "") && !error.school_name_error && !(teacher.school_name === "")) : true;

        const isCollegeValid =
            selectedEntity === "College" ?
                (!error.course_id_error && !(teacher.course_id === "") && !error.institution_id_error && !(teacher.institution_id === "")) : true;

                console.log(!error.class_id_error , !(teacher.class_id === "") , !error.school_name_error , !(teacher.school_name === "")) ;
        if (
            !error.first_name_error && !(teacher.first_name === "") &&
            !error.last_name_error && !(teacher.last_name === "") &&
            !error.email_id_error && !(teacher.email_id === "") &&
            !error.phone_no_error && !(teacher.phone === "") &&
            !error.address_error && !(teacher.address === "") &&
            !error.country_error && !(teacher.country === "") &&
            !error.state_error && !(teacher.state === "") &&
            !error.district_error && !(teacher.district === "") &&
            !error.city_error && !(teacher.city === "") &&
            !error.pincode_error && !(teacher.pincode === "") &&
            !error.qualifications_error && !(teacher.qualification === "") &&
            !error.teaching_experience_error && !(teacher.experience === "") &&
            !error.subject_name_error && (teacher.subjects.length > 0) &&isCollegeValid &&isSchoolValid
            ...
            console.log(payload);
            postRegisterData(getTeacherURL, payload).then((response) => {
                ...
                if (response.status === 200) {
                    ...
                    alert("Teacher registered request sended successfully please wait for 24-48 hours")
    ...
    const handelSubjectChange = (event: SelectChangeEvent<typeof selectedSubject>) => {
            ...
            <div className="access1-card" >
                <div className="card-body">
                    ...
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.first_name_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid first name.</p>
                            )}
                        </div>
                        <div className="col-md-6 col-12 mb-3">
                            <label className="col-form-label fw-bold">
                            ...
                            {error.last_name_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid last name.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                    ...
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.phone_no_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid mobile NUmber.</p>
                            )}
                        </div>
                        <div className="col-md-6 col-12 mb-3">
                            <label className="col-form-label fw-bold">
                            ...
                            {error.email_id_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Email Id.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                        ...
                        {selectedEntity === "School" ?
                            <div className="col-md-6 col-12 mb-3">
                                ...
                                <FormControl sx={{ width: 280 }}>
                                    ...
                                    >
                                        {dataClass.map((item) => (
                                            <MenuItem key={item.id} value={item.id}>{item.class_name}</MenuItem>
                                        ))}
                                    </Select>
                                </FormControl>
                                {error.class_id_error === true && (
                                    <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Class.</p>
                                )}
                            </div> :
                            ...
                            <div className="col-md-6 col-12 mb-3">
                                ...
                                <FormControl sx={{ width: 280 }}>
                                    <InputLabel id="demo-multiple-name-label">Qualification</InputLabel>
                                    <Select
                                        labelId="demo-multiple-name-label"
                                        id="demo1-multiple-name"
                                        name="qualification"
                        ...
                        : <div className="row d-flex justify-content-center">
                            <div className="col-md-6 col-12 mb-3">
                                ...
                                {error.institution_id_error === true && (
                                    <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Institute Name.</p>
                                )}
                            </div>
                            <div className="col-md-6 col-12 mb-3">
                                <label className="col-form-label fw-bold">
                                ...
                                {error.course_id_error === true && (
                                    <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Course.</p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            >
                                ...
                                >
                                    {totleSubject.map((subject: any) => (
                                        ...
                                        >
                                            <Checkbox
                                                checked={selectedSubject.indexOf((subject.subject_id).toString()) > -1}
                            ...
                            {error.subject_name_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Subject Name.</p>
                            )}
                        </div>
                        <div className="col-md-6 col-12 mb-3">
                            <label className="col-form-label fw-bold">
                            ...
                            {error.teaching_experience_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Teaching Experience.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.country_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please select a Country.</p>
                            )}
                        </div>
                        ...
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.state_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please select a state.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.district_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid District.</p>
                            )}
                        </div>
                        <div className="col-md-6 col-12 mb-3">
                            <label className="col-form-label fw-bold">
                            ...
                            {error.city_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid City.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                        <div className="col-md-6 col-12 mb-3">
                            ...
                            {error.address_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Address.</p>
                            )}
                        </div>
                        <div className="col-md-6 col-12 mb-3">
                            <label className="col-form-label fw-bold">
                            ...
                            {error.pincode_error === true && (
                                <p className="error-text " style={{ color: 'red' }}>Please Enter a valid Pincode.</p>
                            )}
                        </div>
                    </div>
                    <div className="row d-flex justify-content-center">
                    ...
                    <div className=" d-flex justify-content-center m-2">
                        <Button variant="contained" onClick={handleSubmit} disabled={CheckTermandcondi} >Submit Form</Button>
                    </div>
